shuttle-pilot-start = Pilotando la nave
shuttle-pilot-end = Pilotaje detenido

shuttle-console-in-ftl = Actualmente en FTL
shuttle-console-mass = Demasiado grande para FTL
shuttle-console-prevent = No es posible pilotar esta nave
shuttle-console-in-expedition = El FTL no está disponible durante las expediciones
shuttle-console-no-powered-ftl-drive = No se detectó ningún motor FTL
shuttle-console-ftl-drive-unpowered = Motor FTL detectado pero sin alimentación

# NAV

shuttle-console-display-label = Pantalla

shuttle-console-position = Posición:
shuttle-console-position-value = {$X}, {$Y}
shuttle-console-orientation = Orientación:
shuttle-console-orientation-value  = {$angle}
shuttle-console-linear-velocity = Velocidad lineal:
shuttle-console-linear-velocity-value = {$X}, {$Y}
shuttle-console-angular-velocity = Velocidad angular:
shuttle-console-angular-velocity-value = {$angularVelocity}
shuttle-console-altitude = Altitud:
shuttle-console-altitude-value = {$altitude}
shuttle-console-vertical-velocity = Velocidad vertical:
shuttle-console-vertical-velocity-value = {$velocity}
shuttle-console-travel-state = Estado:
shuttle-console-travel-state-grounded = En tierra
shuttle-console-travel-state-flying = En vuelo
shuttle-console-travel-state-hovering = Flotando
shuttle-console-travel-state-launching = Despegando ({$countdown})

shuttle-console-unknown = Desconocido
shuttle-console-iff-label = {$name} ({$distance}m)
shuttle-console-track-unknown-label = ID.RASTREO DESCONOCIDO
shuttle-console-track-label = ID.RASTREO
shuttle-console-exclusion = Zona de exclusión

# Buttons
shuttle-console-strafing = Modo de desplazamiento lateral
shuttle-console-nav-settings = Ajustes
shuttle-console-iff-toggle = Mostrar IFF
shuttle-console-iff-detailed = IFF detallado
shuttle-console-dock-toggle = Mostrar atraques
shuttle-console-iffshuttles-toggle = Mostrar transbordadores
shuttle-console-iff-toggle-short = IFF
shuttle-console-iff-detailed-short = IFF DET.
shuttle-console-dock-toggle-short = ATRAQUES

# MAP

shuttle-console-ftl-label = Estado FTL
shuttle-console-ftl-state-Available = Disponible
shuttle-console-ftl-state-Starting = Iniciando
shuttle-console-ftl-state-Travelling = En tránsito
shuttle-console-ftl-state-Arriving = Llegando
shuttle-console-ftl-state-Cooldown = Enfriamiento
shuttle-console-ftl-state-Invalid = Inválido

shuttle-console-map-settings = Ajustes
shuttle-console-ftl-button = FTL
shuttle-console-map-rebuild = Escanear
shuttle-console-map-beacons = Mostrar balizas

shuttle-console-no-signal = Sin señal

shuttle-console-map-objects = Objetos del sector

# DOCK
shuttle-console-docked = Objetos acoplados

shuttle-console-view = Ver
shuttle-console-undock = Desacoplar
shuttle-console-undock-all = Desacoplar todo
shuttle-console-ftl-lock = Bloqueo FTL
shuttle-console-ftl-lock-enabled = Activado
shuttle-console-ftl-lock-disabled = Desactivado
shuttle-console-dock = Acoplar
shuttle-console-docks-label = Atraques

shuttle-console-undock-fail = Error al desacoplar
shuttle-console-dock-fail = Error al acoplar
